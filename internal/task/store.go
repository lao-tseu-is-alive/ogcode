...
func (s *Store) Create(task *Task) error {
	...
	_, err = s.db.Exec(
		`INSERT INTO task (id, plan_id, session_id, parent_task_id, title, description, effort, complexity, status, dependencies, branch_name, worktree_path, pr_url, pr_number, pr_error, order_index, time_created, time_updated)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		task.ID, task.PlanID, task.SessionID, task.ParentTaskID, task.Title, task.Description, task.Effort, task.Complexity, task.Status, string(depsJSON), task.BranchName, task.WorktreePath, task.PRURL, task.PRNumber, task.PRError, task.OrderIndex, task.CreatedAt, task.UpdatedAt,
	)
	return err
}
...
func (s *Store) Get(id string) (*Task, error) {
	row := s.db.QueryRow(
		`SELECT id, plan_id, session_id, parent_task_id, title, description, effort, complexity, status, dependencies, branch_name, worktree_path, pr_url, pr_number, pr_error, order_index, time_created, time_updated
	...
	var depsJSON string
	err := row.Scan(&t.ID, &t.PlanID, &sessionID, &parentTaskID, &t.Title, &t.Description, &t.Effort, &t.Complexity, &t.Status, &depsJSON, &t.BranchName, &t.WorktreePath, &t.PRURL, &prNumber, &t.PRError, &t.OrderIndex, &t.CreatedAt, &t.UpdatedAt)
...
func (s *Store) ListByPlan(planID string) ([]*Task, error) {
	...
	for rows.Next() {
		...
		var prNumber sql.NullInt64
		var depsJSON string
		if err := rows.Scan(&t.ID, &t.PlanID, &sessionID, &parentTaskID, &t.Title, &t.Description, &t.Effort, &t.Complexity, &t.Status, &depsJSON, &t.BranchName, &t.WorktreePath, &t.PRURL, &prNumber, &t.PRError, &t.OrderIndex, &t.CreatedAt, &t.UpdatedAt); err != nil {
			return nil, err
		}
		if sessionID.Valid {
			t.SessionID = &sessionID.String
...
func (s *Store) Update(task *Task) error {
	...
	_, err = s.db.Exec(
		`UPDATE task SET session_id = ?, parent_task_id = ?, title = ?, description = ?, effort = ?, complexity = ?, status = ?, dependencies = ?, branch_name = ?, worktree_path = ?, pr_url = ?, pr_number = ?, pr_error = ?, order_index = ?, time_updated = ? WHERE id = ?`,
		task.SessionID, task.ParentTaskID, task.Title, task.Description, task.Effort, task.Complexity, task.Status, string(depsJSON), task.BranchName, task.WorktreePath, task.PRURL, task.PRNumber, task.PRError, task.OrderIndex, task.UpdatedAt, task.ID,
	)
	return err
}
